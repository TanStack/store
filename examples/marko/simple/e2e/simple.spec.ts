...
test('simple: renders and counts a click', async ({ page }) => {
  ...
  await page.goto('/')
  await expect(page.getByText('dogs: 0')).toBeVisible()
  await page.getByRole('button', { name: 'My Friend Likes dogs' }).click()
  await expect(page.getByText('dogs: 1')).toBeVisible()
